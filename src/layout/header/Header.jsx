import React, {useEffect, useState} from 'react'
import { authenticated } from '../../utils/AuthHelpers';
import { BsChatLeft } from "react-icons/bs";
import { MdGroups2 } from "react-icons/md";
import { BiLogOutCircle } from "react-icons/bi";
import { useNavigate } from 'react-router-dom';

function BasicExample() {

  const [user, setUser] = useState();
  const defaultAvatar = 'data:image/png;base64,';
  const navigate = useNavigate();

  const setCurrentUser = async () => {
    const data = await authenticated()
    setUser(data);
  }

  const openChat = () => {
    // navigate('/')
  }

  const openGroupChat = () => {
    // navigate('/')
  }

  const logout = () => {
    localStorage.removeItem('token');
    navigate(0)
  }

  useEffect(() => {
    setCurrentUser();
  }, [])

  return (
    <div style={{width: '100px'}} className='d-flex flex-column align-items-center vh-100 px-4 py-4 border-end border-1'>
      <img style={{ height: '50px', width: '50px' }} className="rounded-circle border border-2 mb-5" src={user?.avatar ?? defaultAvatar} alt="" />
      <div className='p-3 bg-light rounded-circle mb-4 mt-4 d-flex justify-content-center align-items-center'>
        <BsChatLeft onClick={openChat} style={{cursor: 'pointer'}} className='fs-3' />
      </div>
      <div className=''>
        <MdGroups2 onClick={openGroupChat} style={{cursor: 'no-drop'}} className='fs-3' />
      </div>
      <BiLogOutCircle onClick={logout} style={{cursor: 'pointer'}} className='fs-3 mt-auto' />
    </div>
  );
}

export default BasicExample;