...
const Chat = () => {
  ...
  const id = 1, roomId = 1, isPersonal = 1
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (input) {
      socket.emit('sendMessage', { id, username: user.first_name, roomId, message: input });
  ...
  const getMessages = async () => {
    try {
      const { data: { data } } = await axios.get(`${API.endpoint}/message/get-message/${roomId}`, getHttpOptions());
      setMessages(data);
    } catch (error) {
      console.log(error);
    }
